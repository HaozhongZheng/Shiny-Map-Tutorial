...
homeless_tidy <- read.csv("www/homeless_tidy.csv")
...
`ggsave(filename = paste0(stringr::str_pad(i, 3, pad = "0"), ".png"), path = "www/homeless/")`
...
`imgs <- list.files("www/homeless", full.names = TRUE)`
`lapply(imgs, image_read) %>%`
  `image_join() %>%`
  `image_animate(fps = 5) %>%  # can change speed of gif`
  `image_write("www/homeless.gif")`
  

![Homeless](www/homeless.gif)
...
```{r,echo=FALSE}
events = read.csv("www/Events.csv")
